...
from src.api.utils.agent_trace_broadcaster import (
    emit_trace_start,
    emit_action_start,
    emit_action_update,
    ...
    try:
        while True:
            ...
            try:
                ...
                # Start agent trace for real-time monitoring
                trace_id = f"chat_{session_id}_{uuid.uuid4().hex[:8]}"
                emit_trace_start(trace_id, message, {
                    "session_id": session_id,
                    "kb_name": kb_name,
                    "enable_rag": enable_rag,
                    "enable_web_search": enable_web_search,
                ...
                if enable_rag and kb_name:
                    # Emit RAG agent action
                    rag_action_id = f"rag_{uuid.uuid4().hex[:8]}"
                    emit_action_start(
                        trace_id, rag_action_id, "research",
                        "knowledge_search", f"Searching knowledge base: {kb_name}",
                        {"kb_name": kb_name, "query": message[:100]}
                    )
                    await websocket.send_json(
                        {
                            "type": "status",
                ...
                if enable_web_search:
                    # Emit web search agent action
                    web_action_id = f"web_{uuid.uuid4().hex[:8]}"
                    emit_action_start(
                        trace_id, web_action_id, "research",
                ...
                # Emit tutor agent action for response generation
                tutor_action_id = f"tutor_{uuid.uuid4().hex[:8]}"
                emit_action_start(
                    trace_id, tutor_action_id, "tutor",
                    "generate_response", "Generating personalized response",
                    {"message": message[:100], "history_length": len(history)}
                )
                ...
                async for chunk_data in stream_generator:
                    if chunk_data["type"] == "chunk":
                        ...
                        # Emit streaming update to trace
                        emit_action_update(trace_id, tutor_action_id, "streaming", streaming_content=chunk_data["content"])
                    elif chunk_data["type"] == "complete":
                        full_response = chunk_data["response"]
                        sources = chunk_data.get("sources", {"rag": [], "web": []})

                # Complete trace actions
                if enable_rag and kb_name:
                    emit_action_complete(trace_id, rag_action_id, {"sources_found": len(sources.get("rag", []))})
                if enable_web_search:
                    emit_action_complete(trace_id, web_action_id, {"sources_found": len(sources.get("web", []))})
                emit_action_complete(trace_id, tutor_action_id, {"response_length": len(full_response)})
                ...
                # Complete the trace
                emit_trace_complete(trace_id, {"response_length": len(full_response), "session_id": session_id})

                logger.info(f"Chat completed: session={session_id}, {len(full_response)} chars")

            except Exception as e:
                logger.error(f"Chat processing error: {e}")
                # Emit error to trace if it exists
                if 'trace_id' in locals():
                    emit_action_error(trace_id, tutor_action_id if 'tutor_action_id' in locals() else "unknown", str(e))
                    emit_trace_complete(trace_id, {"error": str(e)})
