...
async def websocket_solve(websocket: WebSocket):
    ...
    try:
        ...
        trace_id = f"solve_{task_id}_{uuid.uuid4().hex[:8]}"
        emit_trace_start(trace_id, question, {
            "task_id": task_id,
            "kb_name": kb_name,
        })

        # Emit manager agent action
        manager_action_id = f"manager_{uuid.uuid4().hex[:8]}"
        emit_action_start(
            trace_id, manager_action_id, "manager",
            "coordinate_solving", "Analyzing problem and coordinating agents",
        ...
        agent_action_ids = {}  # Track action IDs for each agent
        ...
        if hasattr(solver.logger, "display_manager") and solver.logger.display_manager:
            ...
            def wrapped_set_status(agent_name: str, status: str):
                ...
                # Map agent names to trace agent types
                agent_type_map = {
                ...
                agent_type = agent_type_map.get(agent_name, "tutor")
                
                # Emit action based on status
                if status in ["working", "thinking", "processing"]:
                    action_id = f"{agent_name.lower()}_{uuid.uuid4().hex[:6]}"
                    agent_action_ids[agent_name] = action_id
                    emit_action_start(
                        trace_id, action_id, agent_type,
                        f"{agent_name.lower()}_processing", f"{agent_name} agent is working",
                        {"status": status}
                    )
                elif status in ["done", "complete", "finished", "idle"]:
                    if agent_name in agent_action_ids:
                        emit_action_complete(trace_id, agent_action_ids[agent_name], {"status": status})
                elif status == "error":
                    if agent_name in agent_action_ids:
                        emit_action_error(trace_id, agent_action_ids[agent_name], "Agent encountered an error")
        ...
        try:
            ...
            with interceptor:
                ...
                # Complete manager action and trace
                emit_action_complete(trace_id, manager_action_id, {"status": "completed"})
            ...
            emit_action_error(trace_id, manager_action_id, str(e))
            emit_trace_complete(trace_id, {"error": str(e), "status": "error"})
        finally:
