#!/usr/bin/env python
"""
Web Search Tool - Network search using Perplexity API, Baidu AI Search API, or Exa AI
"""
...
def _search_with_exa(query: str, num_results: int = 10, verbose: bool = False) -> dict:
    """
    ...
    if not EXA_AVAILABLE:
        raise ImportError(
            "exa_py module is not installed. To use Exa search, please install: "
            "pip install exa-py"
        )
    ...
    if Exa is None:
        raise ImportError("Exa module is not available")
    ...
    result = {
        ...
        "autoprompt_string": (
            search_response.autoprompt_string if hasattr(search_response, "autoprompt_string") else None
        ),
    }

    if verbose:
        print(f"[Exa AI] Query: {query}")
        print(f"[Exa AI] Results count: {len(search_response.results)}")
        if hasattr(search_response, "autoprompt_string") and search_response.autoprompt_string:
            print(f"[Exa AI] Autoprompt: {search_response.autoprompt_string}")
...
def web_search(
    ...
    Args:
        ...
        exa_num_results: Number of results to return for Exa (default: 10)
    ...
    try:
        if provider == "baidu":
            ...
            result = _search_with_exa(query=query, num_results=exa_num_results, verbose=verbose)
        else:
            raise ValueError(
                f"Unsupported search provider: {provider}. Use 'perplexity', 'baidu', or 'exa'."
            )
