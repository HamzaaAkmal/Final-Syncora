...
export default function KnowledgePage() {
  ...
  const [selectedKbForChat, setSelectedKbForChat] = useState<string>("");
  ...
  const handleAskQuestion = async () => {
    if (!inputQuestion.trim() || !selectedKbForChat) return;
    ...
    try {
      ...
      const response = await fetch(apiUrl("/api/v1/knowledge/query"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          kb_name: selectedKbForChat,
          question: question,
          top_k: 5,
        }),
      });
      ...
      const assistantMessage: Message = {
        ...
        role: "assistant",
        content: data.answer || "No answer found. Please ensure the knowledge base is initialized.",
        timestamp: new Date(),
      };
  ...
  const openChatModal = (kbName: string) => {
    setSelectedKbForChat(kbName);
    setMessages([
      {
        id: "welcome",
        role: "assistant",
        content: `✅ Connected to knowledge base "${kbName}"!\n\nAsk me any questions about the content in this knowledge base.`,
        timestamp: new Date(),
      },
    ]);
    setQueryError("");
  ...
  return (
    <div className="animate-fade-in h-screen overflow-y-auto p-6" style={{ backgroundColor: '#050505' }}>
      {/* Header */}
              ...
              <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-start">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {kb.is_default && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/40 text-[10px] font-bold uppercase tracking-wide border border-emerald-100 dark:border-emerald-800 mt-1" style={{ color: '#10B981' }}>
                        Default
                      </span>
                    )}
                  </div>
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => openChatModal(kb.name)}
                    disabled={!kb.statistics.rag_initialized}
                    className="p-2 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 rounded-lg text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    title={kb.statistics.rag_initialized ? "Chat with Knowledge Base" : "Initialize RAG first"}
                  >
                    ...
                    {(() => {
                      ...
                      if (progress) {
                        if (progress.stage === "completed") {
                          ...
                          return (
                            <span className="font-bold flex items-center gap-1" style={{ color: '#10B981' }}>
              ...
              <div>
                ...
                >
                  ...
                  >
                    <Upload
                      className="w-8 h-8"
                      style={{ color: dragActive ? '#10B981' : undefined }}
                    />
      ...
      {/* Chat Modal - RAG Query Interface */}
      {chatModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0, 0, 0, 0.8)' }}>
          <div className="w-full max-w-4xl h-[80vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden" style={{ backgroundColor: '#0F0F0F', border: '1px solid #1F1F1F' }}>
            {/* Chat Header */}
            <div className="px-6 py-4" style={{ backgroundColor: '#161616', borderBottom: '1px solid #1F1F1F' }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <MessageCircle className="w-6 h-6" style={{ color: '#10B981' }} />
                  <div>
                    <h2 className="text-xl font-bold" style={{ color: '#F8FAFC' }}>
                      Chat with {selectedKbForChat}
                    </h2>
                    <p className="text-sm" style={{ color: 'rgba(248, 250, 252, 0.6)' }}>
                      Ask questions about the content in this knowledge base
                    </p>
                  </div>
                </div>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {messages.map((msg) => (
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundColor: msg.role === "user" ? '#10B981' : '#161616',
                      color: msg.role === "user" ? '#050505' : '#F8FAFC',
                  ...
                  >
                    ...
                    <p
                      ...
                      style={{
                        color: msg.role === "user" ? 'rgba(5, 5, 5, 0.6)' : 'rgba(248, 250, 252, 0.5)'
                      }}
                    >
              ...
              {isQuerying && (
                <div className="flex justify-start">
                  <div className="px-4 py-3 rounded-lg" style={{ backgroundColor: '#161616', border: '1px solid #1F1F1F' }}>
                    <Loader2 className="animate-spin" style={{ color: '#10B981' }} />
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Input Area */}
            <div className="p-6" style={{ backgroundColor: '#161616', borderTop: '1px solid #1F1F1F' }}>
              <div className="flex gap-3">
                <input
                  type="text"
                  value={inputQuestion}
                  ...
                  disabled={isQuerying}
                  className="flex-1 px-4 py-3 rounded-lg focus:outline-none transition-all"
                  style={{
                ...
                <button
                  ...
                  className="px-6 py-3 rounded-lg flex items-center gap-2 hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                  style={{
              ...
              {queryError && (
                <p className="text-sm mt-2" style={{ color: '#EF4444' }}>❌ {queryError}</p>
              )}
            </div>
          </div>
        </div>
